...
const formatSolDelta = (deltaLamports: number) => {
  if (deltaLamports === 0) {
    return "+0.000000";
  }
  const sign = deltaLamports < 0 ? "-" : "+";
  const absLamports = Math.abs(deltaLamports);
  return `${sign}${formatLamports(absLamports)}`;
};
...
const postJson = async <T>(
  ...
  if (!response.ok) {
    ...
    throw new Error(`offchain ${response.status}: ${detail}`);
  }
  return payload as T;
};

const ensureOffchainReady = async (baseUrl: string) => {
  const response = await fetch(`${baseUrl}/health`);
  if (!response.ok) {
    throw new Error(
      `Offchain backend is not reachable at ${baseUrl} (${response.status}).`
    );
  }
};

const offchainSignIn = async (
  baseUrl: string,
  label: string,
  keypair: Keypair
) => {
  ...
  const challenge = await postJson<{ nonce: string; message: string }>(
    `${baseUrl}/v1/auth/challenge`,
    { wallet }
  );
  const signature = bs58.encode(
    nacl.sign.detached(Buffer.from(challenge.message, "utf8"), keypair.secretKey)
  );
  const verify = await postJson<{
    token: string;
    user: { wallet: string; handle: string; role: string };
  }>(`${baseUrl}/v1/auth/verify`, {
    wallet,
    nonce: challenge.nonce,
    signature,
  });
  console.log(`${label} handle:`, verify.user.handle);
  return verify;
};

const offchainCreateInvite = async (
  baseUrl: string,
  token: string,
  inviteeRole: "client" | "contractor"
) => {
  return postJson<{
    inviteId: string;
    token: string;
    ...
    contractId: string;
    expiresAt: number;
  }>(`${baseUrl}/v1/invites`, { inviteeRole }, token);
};

const offchainAcceptInvite = async (
  baseUrl: string,
  token: string,
  inviteToken: string
) => {
  return postJson<{ contractId: string; status: string }>(
    `${baseUrl}/v1/invites/accept`,
    { token: inviteToken },
    token
  );
};

const offchainLinkEscrow = async (
  baseUrl: string,
  token: string,
  contractId: string,
  escrowPda: PublicKey,
  ...
  return postJson(
    `${baseUrl}/v1/contracts/${contractId}/link-escrow`,
    {
...
const offchainMarkFunded = async (
  baseUrl: string,
  token: string,
  contractId: string
) => {
  return postJson(
    `${baseUrl}/v1/contracts/${contractId}/mark-funded`,
    {},
    token
  );
};
...
describe("Nebulon Full Local Scenario (offchain + PER enforcement)", () => {
  const rpcUrl =
  ...
  const offchainBaseUrl = normalizeEndpoint(
    process.env.OFFCHAIN_BASE_URL || "http://localhost:3333"
  );

  let offchainContractId = "";
  let clientJwt = "";
  let devJwt = "";
  let adminJwt = "";
  ...
  const netPayment = totalPayment.sub(feeAmount);
  ...
  const ensureUsdcBalance = async (
    ...
    while (true) {
      const balance = await getTokenAmount(connection, account);
      if (balance >= minAmount) {
        return;
      }
      console.log(`${label} needs test USDC.`);
      console.log(fundingNote);
      console.log(`Run: ${mintCommand}`);
      const answer = await promptUser("Press N to recheck or M to cancel: ");
      if (answer === "m") {
        process.exit(0);
      }
    }
  ...
  before(async () => {
    ...
    const requiredUsdc = BigInt(grossFunding.toString());
    const mintAmount = formatUsdcAmount(requiredUsdc);
    const feeUsdc = BigInt(feeAmount.toString());
    const netUsdc = BigInt(netPayment.toString());
    const fundingNote =
      `Contract amount: ${formatUsdcAmount(BigInt(totalPayment.toString()))} TUSDC\n` +
      `Fee (deducted): ${formatUsdcAmount(feeUsdc)} TUSDC\n` +
      `Net to escrow: ${formatUsdcAmount(netUsdc)} TUSDC`;
    const mintCommand = `ANCHOR_PROVIDER_URL=${rpcUrl} npm run mint-tusdc -- ${mintAmount} ${client.publicKey.toBase58()}`;
  ...
  it("runs the new regular flow", async () => {
    ...
    clientSolBefore = await connection.getBalance(client.publicKey);
    devSolBefore = await connection.getBalance(dev.publicKey);
    console.log(
      "client SOL:",
      `${formatLamports(clientSolBefore)} SOL`
    );
    console.log(
      "dev SOL:",
      `${formatLamports(devSolBefore)} SOL`
    );
    ...
    await ensureOffchainReady(offchainBaseUrl);

    console.log("1) Client signs in (offchain)");
    const clientAuth = await offchainSignIn(offchainBaseUrl, "client", client);
    clientJwt = clientAuth.token;

    console.log("2) Client creates invite");
    const invite = await offchainCreateInvite(
      offchainBaseUrl,
      clientJwt,
      "contractor"
    );
    offchainContractId = invite.contractId;
    console.log("invite url:", invite.inviteUrl);

    console.log("3) Dev signs in (offchain)");
    const devAuth = await offchainSignIn(offchainBaseUrl, "dev", dev);
    devJwt = devAuth.token;

    console.log("3.5) Admin signs in (offchain)");
    const adminAuth = await offchainSignIn(offchainBaseUrl, "admin", payerKeypair);
    adminJwt = adminAuth.token;

    console.log("4) Dev accepts invite as dev");
    await offchainAcceptInvite(offchainBaseUrl, devJwt, invite.token);
    ...
    if (!clientJwt || !offchainContractId) {
      throw new Error("offchain auth or contract id missing");
    }
    await offchainLinkEscrow(
      offchainBaseUrl,
      clientJwt,
      offchainContractId,
      escrowPda,
    ...
    // Wait for transaction confirmation and indexer visibility
    console.log("13) Wait for transaction confirmation and indexer visibility");
    await sleep(3000); // 3 seconds for safety

    // Trigger indexer manually to update contract status
    console.log("14) Trigger indexer to update contract status");
    await postJson(
      `${offchainBaseUrl}/v1/admin/indexer/contract/${offchainContractId}/tick`,
      {},
      adminJwt
    );

    await offchainMarkFunded(
      offchainBaseUrl,
      clientJwt,
      offchainContractId
    );
    ...
    const clientSolAfter = await connection.getBalance(client.publicKey);
    const devSolAfter = await connection.getBalance(dev.publicKey);
    console.log(
    ...
    console.log(
      "client SOL:",
      `${formatLamports(clientSolAfter)} SOL (${formatSolDelta(
        clientSolAfter - clientSolBefore
      )} SOL)`
    );
    console.log(
      "dev SOL:",
      `${formatLamports(devSolAfter)} SOL (${formatSolDelta(
        devSolAfter - devSolBefore
      )} SOL)`
    );
