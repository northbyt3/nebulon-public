...
describe("Nebulon New Flow (regular scenario)", () => {
  const rpcUrl =
  ...
  before(async () => {
    ...
    console.log("Funding wallet:", provider.wallet.publicKey.toBase58());
    ...
    console.log("funder SOL after prompt:", funderBalanceAfter);
      ...
      if (existing) {
        try {
          const revokeTx = await sessionTokenManager.program.methods
            ...
            .transaction();
          await provider.sendAndConfirm(revokeTx, [authority]);
          revoked = true;
        } catch {
          revoked = false;
        }
  ...
  it("runs the new regular flow", async () => {
    ...
    const createPrivate0Sig = await program.methods
    ...
    const createPrivate1Sig = await program.methods
      .createPrivateMilestone(
        escrowId,
        1,
        textToHash("placeholder")
      ...
      .signers([devSessionSigner])
      .rpc();
    console.log("createPrivateMilestone 1 sig:", createPrivate1Sig);

    const update0Sig = await program.methods
    ...
    const draftTermsSig = await program.methods
      .createPrivateTerms(
        escrowId,
    ...
    const finalTermsSig = await program.methods
      .createPrivateTerms(
        escrowId,
    ...
    const devSignSig = await program.methods
      .signPrivateTerms(escrowId)
      .accounts({
        user: dev.publicKey,
        payer: devSessionSigner.publicKey,
